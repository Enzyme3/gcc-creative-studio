...
FE_SERVICE_NAME=$(grep 'frontend_service_name' "$TFVARS_FILE" | awk -F'"' '{print $2}')
WEB_APP_ID=$(firebase apps:list --project="$PROJECT_ID" --json | jq -r --arg name "$FE_SERVICE_NAME" '.result[] | select(.displayName == $name) | .appId')

if [ -z "$WEB_APP_ID" ] || [ "$WEB_APP_ID" == "null" ]; then
  WEB_APP_ID=$(firebase apps:list --project="$PROJECT_ID" --json | jq -r '.result[0].appId // ""')
fi
